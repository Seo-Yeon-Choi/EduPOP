...
document.addEventListener("DOMContentLoaded", function () {
    ...
    retryButton.addEventListener("click", loadExp); // 다시 불러오기 버튼을 누르면 경험치 재조회
    ...
    async function loadExp() {
        ...
        try {
            ...
            if (!response.ok) {
                throw new Error(findErrorMessage(response.status));
            }
            ...
            if (responseType === null
                ...
                throw new Error("경험치 응답 형식이 올바르지 않습니다.");
            }

            const expInfo = await response.json();
            // await(어웨이트): 서버가 보낸 JSON 경험치 정보가 도착할 때까지 기다림

            fillExpCard(expInfo); // 조회한 경험치 정보를 카드에 표시
            showContent(); // 로딩 화면을 숨기고 경험치 카드 본문 표시
            ...
            showError(message); // 조회 실패 이유를 오류 영역에 표시
        }
    }


    function fillExpCard(expInfo) {
        const currentStage = readStage(expInfo.characterStage);
        const currentStageName = readStageName(
            expInfo.stageName,
            currentStage
        );
        const currentTotalExp = readPositiveNumber(
            expInfo.totalExp,
            "총 경험치"
        );
        const expToNextStage = readPositiveNumber(
            expInfo.expToNextStage,
            "다음 단계 경험치"
        );
        const progressPercent = readPercent(
            expInfo.expProgressPercent
        );
        const isMaxStage = expInfo.maxStage === true
            || currentStage === 6;

        card.dataset.stage = String(currentStage); // 현재 단계에 맞는 CSS 색상 적용

        stageBadge.textContent = currentStage + "단계"; // 현재 캐릭터 단계 표시
        stageName.textContent = currentStageName; // G부터 GROW UP까지 현재 단계 이름 표시
        totalExp.textContent = formatNumber(currentTotalExp) + " EXP"; // 현재 총 경험치 표시

        if (isMaxStage) {
            nextExp.textContent = "최종 단계를 달성했습니다.";
            progressText.textContent = "최종 단계 달성 100%";
            guide.textContent = "GROW UP을 완성했습니다. 앞으로도 학습 활동을 이어가 보세요.";

            changeProgress(100); // 최종 단계이면 진행 막대를 100%로 표시

        } else {
            nextExp.textContent =
                "다음 단계까지 "
                + formatNumber(expToNextStage)
                + " EXP";
            ...
            changeProgress(progressPercent); // 현재 단계의 경험치 진행률 표시
        }
    ...
    function changeProgress(percent) {
        ...
        progressBar.style.width = percent + "%"; // 경험치 진행 막대 길이 변경
    }


    function changeCharacter(imageUrl, currentStageName) {
        const safeUrl = readImageUrl(
            imageUrl,
        ...
        characterPlaceholder.textContent = currentStageName + " 캐릭터 준비 중";
        ...
        characterImage.alt = currentStageName + " 성장 단계 캐릭터";
        characterImage.src = safeUrl; // 캐릭터 이미지 주소 적용
    }


    function changeBackground(imageUrl, currentStage) {
        const safeUrl = readImageUrl(
            imageUrl,
            "/images/exp/stage" + currentStage + "-background.png"
        );
        ...
        backgroundImage.onload = function () {
            if (card.dataset.stage === String(currentStage)) {
                characterBackground.style.backgroundImage =
                    "url('" + safeUrl + "')";
            }
        ...
        backgroundImage.src = safeUrl; // 단계별 배경 이미지 불러오기
    }
    ...
    function readStage(value) {
        ...
        if (!Number.isInteger(stage)
            ...
            throw new Error("캐릭터 성장 단계 정보가 올바르지 않습니다.");
        }
    ...
    function readStageName(value, stage) {
        const defaultNames = [
            "",
            "G",
            "GR",
        ...
        if (typeof value !== "string"
            ...
            return defaultNames[stage]; // 단계 이름이 비어 있으면 정해진 기본 이름 사용
        }
    ...
    function readPositiveNumber(value, itemName) {
        const number = Number(value);

        if (!Number.isFinite(number) || number < 0) {
            throw new Error(itemName + " 정보가 올바르지 않습니다.");
        }

        return Math.floor(number); // 화면에는 0 이상의 정수 경험치만 사용
    }


    function readPercent(value) {
        const percent = readPositiveNumber(
            value,
            "경험치 진행률"
        );

        return Math.min(percent, 100); // 진행률이 100%를 넘지 않도록 제한
    }


    function readImageUrl(value, defaultUrl) {
        if (typeof value !== "string") {
            return defaultUrl;
        }

        const imageUrl = value.trim();
        const safeImagePath = /^\/images\/exp\/[a-zA-Z0-9_-]+\.png$/;

        if (!safeImagePath.test(imageUrl)) {
            return defaultUrl; // 정해진 경험치 이미지 폴더 밖의 주소는 사용하지 않음
        }
    ...
    function formatNumber(value) {
        return value.toLocaleString("ko-KR"); // 경험치를 1,000처럼 읽기 쉽게 표시
    }
